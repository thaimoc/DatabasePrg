...
namespace DataAccess
{
    public class Product
    {
        ...
        public static List<Product> FindBy(object ValueCbbox, object Value, bool flag)
        {
            string query = "";
            if (flag)
                query = @"select * from View_Products where " + Value + " = " + ValueCbbox + "";
            else
                query = @"select * from View_Products where " + Value + " like '%" + ValueCbbox + "%'";
            return CBO.FillCollection<Product>(DataProvider.Instance.ExecuteReader(query));
        }
        ...
        public static List<Product> FindBy(object value, object cbbvalue)
        {
            string query = "Select * from Products where " + cbbvalue + " like '" + value + "%'";
            return CBO.FillCollection<Product>(DataProvider.Instance.ExecuteReader(query));
        }
    }
}
